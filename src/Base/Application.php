...
class Application
{
    ...
    public function handleRequest(Request $request){
        ...
        // вызом метода dispatch. Метод принимает на вход метод запроса (REQUEST_METHOD)
        // и строку запроса (REQUEST_URI)
        // метод возвращает информацию массивом, в котором содержится обработчик
        // данного запроса (контроллер и его метод)
        // или информация о том, что реакция на данный запрос на описана
        // в config файле
        ...
        // функция call_user_func_array вызывает полученные ранее (40 строка)
        // метод контроллера,
        // в метод передается объект запроса
        // методы - обработчики контроллеров возвращают сформированный ответ
        // (объект Response)
        $response = call_user_func_array(
            $controllerData[0], // [new $controller(), $action]
